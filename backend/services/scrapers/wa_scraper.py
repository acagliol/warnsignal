...
class WAScraper(BaseScraper):
    """Washington State WARN Act scraper.

    WA publishes WARN notices through the Employment Security Department (ESD).
    Data is typically available as downloadable Excel/CSV files at esd.wa.gov.
    """

    STATE = "WA"
    BASE_URL = "https://esd.wa.gov/about-employees/WARN"
    ALT_URLS = [
        "https://esd.wa.gov/about-employees/warn",
        "https://esd.wa.gov/labormarketinfo/warn-data",
        "https://esd.wa.gov/about-employees/WARN-list",
        "https://esd.wa.gov/newsroom/warn-notices",
    ]
    ...
    def scrape(self) -> List[Dict[str, Any]]:
        ...
        try:
            ...
            if parsed:
                self.logger.info(f"WA primary URL: {len(parsed)} filings")
                results.extend(parsed)
        except Exception as e:
            self.logger.warning(f"WA primary URL failed: {e}")
        ...
        if not results:
            for url in self.ALT_URLS:
                try:
                    ...
                    if parsed:
                        self.logger.info(f"WA alt source {url}: {len(parsed)} filings")
                        results.extend(parsed)
                        break
                except Exception as e:
                    self.logger.warning(f"WA alt source {url} failed: {e}")
        ...
        self.logger.info(f"WA scraper found {len(unique)} unique filings")
        return unique

    def _scrape_page(self, resp, page_url: str) -> List[Dict[str, Any]]:
        """Scrape a single WA WARN page for download links and HTML tables."""
        results = []
        ...
        for url in download_links:
            try:
                ...
                self.logger.warning(f"WA failed to parse download {url}: {e}")
        ...
        for c in cols:
            ...
            if "company" in cl or "employer" in cl or "business" in cl or "organization" in cl:
                ...
                if "employees" not in mapping:
                    mapping["employees"] = c
            elif "city" in cl or "location" in cl or "county" in cl or "region" in cl or "lwda" in cl:
