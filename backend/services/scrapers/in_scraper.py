...
class INScraper(BaseScraper):
    """Indiana WARN Act scraper.

    IN publishes WARN notices through the Department of Workforce Development (DWD).
    Data may be available as Excel/CSV downloads or HTML tables.
    """

    STATE = "IN"
    BASE_URL = "https://www.in.gov/dwd/warn-notices/"
    ALT_URLS = [
        "https://www.in.gov/dwd/warn-notices/current-warn-notices/",
        "https://www.in.gov/dwd/warn-notices/archived-warn-notices/",
    ...
    def scrape(self) -> List[Dict[str, Any]]:
        ...
        try:
            ...
            if parsed:
                self.logger.info(f"IN primary URL: {len(parsed)} filings")
                results.extend(parsed)
        except Exception as e:
            self.logger.warning(f"IN primary URL failed: {e}")

        # Try alternate URLs for additional data
        for url in self.ALT_URLS:
            try:
                resp = self._get(url)
                parsed = self._scrape_page(resp, url)
                if parsed:
                    self.logger.info(f"IN alt source {url}: {len(parsed)} filings")
                    results.extend(parsed)
            except Exception as e:
                self.logger.warning(f"IN alt source {url} failed: {e}")
        ...
        self.logger.info(f"IN scraper found {len(unique)} unique filings")
        return unique

    def _scrape_page(self, resp, page_url: str) -> List[Dict[str, Any]]:
        """Scrape a single IN WARN page for download links and HTML tables."""
        results = []
        ...
        self.logger.info(f"IN: found {len(download_links)} potential download links on {page_url}")
        ...
        for url in download_links:
            try:
                ...
                if parsed:
                    self.logger.info(f"IN: {url} -> {len(parsed)} filings")
                    results.extend(parsed)

                time.sleep(self.delay)
            except Exception as e:
                self.logger.warning(f"IN download {url} failed: {e}")
