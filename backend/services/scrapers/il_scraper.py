...
class ILScraper(BaseScraper):
    """Illinois DCEO WARN Act scraper.

    IL publishes WARN notices via HTML tables or downloadable files.
    Falls back to PDF parsing if needed.
    """

    STATE = "IL"
    BASE_URL = "https://dceo.illinois.gov/workforcedevelopment/warn.html"
        ...
        try:
            ...
            for link in soup.find_all("a", href=True):
                ...
                if any(ext in href.lower() for ext in [".xlsx", ".xls", ".csv"]):
                    full_url = href if href.startswith("http") else f"https://dceo.illinois.gov{href}"
                    try:
                    ...
                    try:
                        ...
                        self.logger.warning(f"Failed to parse IL PDF {full_url}: {e}")
            ...
            self.logger.error(f"IL scraper failed: {e}")

        self.logger.info(f"IL scraper found {len(results)} filings")
        return results
