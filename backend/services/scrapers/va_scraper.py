...
class VAScraper(BaseScraper):
    """Virginia WARN Act scraper.

    VA publishes WARN notices through the Virginia Employment Commission (VEC)
    and VirginiaWorks. Data may be available as HTML tables or downloadable files.
    """

    STATE = "VA"
    BASE_URL = "https://www.vec.virginia.gov/warn-notices"
    ALT_URLS = [
        "https://www.virginiaworks.gov/warn-notices/",
        "https://www.vec.virginia.gov/employers/warn-notices",
        "https://www.vec.virginia.gov/warn",
        "https://www.doli.virginia.gov/warn-notices/",
    ]
    ...
    def scrape(self) -> List[Dict[str, Any]]:
        ...
        try:
            ...
            if parsed:
                self.logger.info(f"VA primary URL: {len(parsed)} filings")
                results.extend(parsed)
        except Exception as e:
            self.logger.warning(f"VA primary URL failed: {e}")
        ...
        if not results:
            for url in self.ALT_URLS:
                try:
                    ...
                    if parsed:
                        self.logger.info(f"VA alt source {url}: {len(parsed)} filings")
                        results.extend(parsed)
                        break
                except Exception as e:
                    self.logger.warning(f"VA alt source {url} failed: {e}")
        ...
        self.logger.info(f"VA scraper found {len(unique)} unique filings")
        return unique

    def _scrape_page(self, resp, page_url: str) -> List[Dict[str, Any]]:
        """Scrape a single VA WARN page for download links and HTML tables."""
        results = []
        ...
        for url in download_links:
            try:
                ...
                self.logger.warning(f"VA failed to parse download {url}: {e}")
        ...
        for c in cols:
            ...
            if "company" in cl or "employer" in cl or "business" in cl or "organization" in cl:
                ...
                if "employees" not in mapping:
                    mapping["employees"] = c
            elif "city" in cl or "location" in cl or "county" in cl or "locality" in cl:
